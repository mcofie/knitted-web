...
function OrderTimeline({status, created_at}: { status: Order['status'], created_at: string }) {
    if (status === 'cancelled') {
        return (
            <div
                className="rounded-lg bg-red-50 border border-red-100 p-4 flex items-center gap-3 text-red-700 dark:bg-red-900/10 dark:border-red-900/20 dark:text-red-400">
        ...
        <div className="w-full py-4">
            ...
            <div className="mt-4 text-center">
                <p className="text-xs text-muted-foreground">
                    Ordered on {new Date(created_at).toLocaleDateString(undefined, {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                })}
        ...
        <main className="min-h-screen bg-[#FAFAFA] dark:bg-[#09090b] text-foreground flex flex-col">
            ...
            <header className="bg-background border-b sticky top-0 z-20 flex-none">
                <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
                    ...
                    {order.ready_at && (
                        <div
                            className="hidden sm:block text-xs font-medium text-primary hover:underline cursor-pointer">
                            <AddToCalendarButton
                    ...
                    <div className="lg:col-span-8 space-y-8">
                        ...
                        <div className="bg-background rounded-xl border p-6">
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1">Order
                                        Number</p>
                            ...
                            {order.ready_at && (
                                <div
                                    className="mt-6 bg-secondary/30 rounded-lg p-4 flex items-start gap-3 border border-secondary/50">
                                    <Clock className="w-5 h-5 text-primary shrink-0 mt-0.5"/>
                                    <div className="flex-1">
                                        <p className="text-sm font-medium text-foreground">Estimated Completion</p>
                                        <p className="text-xs text-muted-foreground">
                                            This order is scheduled to be ready by <span
                                            className="font-semibold text-foreground">{new Date(order.ready_at).toLocaleString(undefined, {
                                            dateStyle: 'medium',
                                            timeStyle: 'short'
                                        })}</span>.
                                        </p>
                                ...
                                {items.map((item) => {
                                    ...
                                    return (
                                        <div key={item.id}
                                            ...
                                            <div
                                                className="h-12 w-12 rounded-md bg-muted flex items-center justify-center shrink-0">
                                            ...
                                            <div className="text-right">
                                                <p className="text-sm font-medium"><Money amount={lineTotal}
                                                                                          currency={item.currency_code || currency}/>
                                                </p>
                                                {item.quantity > 1 && (
                                                    <p className="text-[10px] text-muted-foreground">
                                                        <Money amount={item.unit_price ?? 0}
                                                               currency={item.currency_code || currency}/> ea
                                                    </p>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    {attachments.map((att) => (
                                        ...
                                        >
                                            {att.kind === 'image' ? (
                                                ...
                                                <img src={att.signed_url || ""} alt="Design"
                                            ...
                                            <div
                                                ...
                                                className="text-xs font-medium">View</span>
                                            </div>
                                        </a>
                                    ))}
                                </div>
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <div className="bg-white dark:bg-card rounded-xl border p-0 overflow-hidden sticky top-24">
                            ...
                            <div className="p-5 bg-muted/30">
                                ...
                                <div className={cn(
                                    ...
                                    <span
                                        className="text-xs font-bold uppercase">{finalBalance > 0 ? "Due" : "Settled"}</span>
                            ...
                            {payments.length > 0 && (
                                <div className="border-t px-5 py-4">
                                    <p className="text-xs font-medium text-muted-foreground mb-3 uppercase tracking-wide">Payment
                                        History</p>
                                        ...
                                        {payments.map((p) => (
                                            <li key={p.id} className="flex justify-between items-center text-xs">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span
                                                        className="capitalize text-foreground">{p.method || "Payment"}</span>
                                                </div>
                        ...
                        <div className="bg-white dark:bg-card rounded-xl border p-5 overflow-hidden">
                            ...
                            {customer ? (
                                <div className="space-y-3 text-sm">
                                    <div
                                        className="font-medium text-foreground text-base">{customer.name || order.customer_name}</div>
            ...
            <footer className="mt-12 border-t bg-muted/20 flex-none">
                <div className="py-12 px-4 text-center">
                    <div className="max-w-md mx-auto space-y-4">
                        <div
                            className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-primary/10 mb-2">
                            <Scissors className="w-6 h-6 text-primary"/>
                        </div>
                        <h3 className="text-xl font-bold tracking-tight">Run your own tailoring business?</h3>
                        <p className="text-sm text-muted-foreground leading-relaxed">
                            Use Knitted to track orders, manage measurements, and send professional tracking links like
                            this one to your clients.
                        </p>
                        <div className="pt-2">
                            <Link
                                href="https://getknitted.app"
                                target="_blank"
                ...
                <div className="border-t py-6 text-center">
                    <p className="text-xs text-muted-foreground">
                        &copy; {new Date().getFullYear()} Knitted. Made in <span
                        className="text-foreground font-medium">Accra</span>.
                    </p>
                </div>
            </footer>
        </main>
