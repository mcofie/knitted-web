...
export default async function ClientsPage({searchParams}: Props) {
    ...
    const pageSize = Math.min(100, Math.max(6, Number(searchParams.pageSize ?? 12)));
                        ...
                        <CardHeader className="pb-2">
                            <CardTitle className="text-base">
                                <Link href={`/clients/${c.id}`}
                                      className="hover:underline">{c.full_name ?? c.name}</Link>
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="text-sm text-muted-foreground space-y-1">
                            <div>{c.email ?? '—'}</div>
                            <div>{c.phone ?? '—'}</div>
                            <div>{c.city ?? '—'} {c.country_code ? `• ${c.country_code}` : ''}</div>
                        </CardContent>
                    </Card>
                ))}
                {clients?.length === 0 && (
                    <Card><CardContent className="py-10 text-center text-muted-foreground">No clients yet. Add your
                        first one.</CardContent></Card>
                )}
            </div>
