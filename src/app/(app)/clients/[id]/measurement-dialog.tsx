...
const Schema = z.object({
    ...
    // IMPORTANT: plain number (no coerce), we’ll use valueAsNumber in the input
    value: z.number().gt(0, { message: "Value must be > 0" }),
    // Keep the UI’s explicit set of units
    ...
    useEffect(() => {
        if (editRow) {
            reset({
                ...
                unit: ((editRow.unit as "in" | "cm" | null) ?? "in") as "in" | "cm",
            });
        } else {
            ...
            if (editRow) {
                const { error } = await sb
                    ...
                    .update({
                        ...
                        // include input_unit if your schema requires it:
                        // input_unit: values.unit ?? null,
                    })
                    .eq("id", editRow.id);
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                        ...
                        {SUGGESTIONS.map((label) => {
                            ...
                            return (
                                <Button
                                    ...
                                    size="sm"
                                    className={clsx("rounded-full", active && "ring-2 ring-ring")}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 pt-2" noValidate>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="space-y-1">
                            ...
                            <Input
                                ...
                                type="number"
                                step="0.01"
                                // CRUCIAL: this makes RHF send a number to the resolver
